...
public class LexicalAnalyzer {
    ...
    public boolean lexError(String str){							//判斷結尾是否是";"
    ...
    public boolean loopError(){										//判斷for+if+while-end=0
    	int loop_Num=for_Num+if_Num+while_Num;
	...
	public static void main(String[] args) {
		...
		try{
			...
			while((line=br.readLine())!=null){
				if(LexicalAnalyzer.lexError(line)==false){
					break;
				}else{
		...
		if(LexicalAnalyzer.getError_Var()==true&&LexicalAnalyzer.loopError()==true
				&&LexicalAnalyzer.typeError()==true&&LexicalAnalyzer.getError_Flag()){						
